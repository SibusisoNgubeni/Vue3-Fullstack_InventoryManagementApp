...
export const addItem = async (c: Context): Promise<Response> => {
    ...
    if (!name || !description || !price) {
        return c.json({ success: false, message: 'Name, description, and price are required.' }, 400);
    }

    try {
        const newItem = await createFoodItem(name, description, price);
...
export const updateItem = async (c: Context): Promise<Response> => {
    ...
    if (!name && !description && !price) {
        return c.json({ success: false, message: 'At least one field (name, description, price) is required for update.' }, 400);
    }

    try {
        const updatedItem = await updateFoodItem(Number(id), name, description, price);
        if (updatedItem) {
